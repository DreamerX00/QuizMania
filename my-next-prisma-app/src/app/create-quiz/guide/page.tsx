...
export default function CreateQuizGuidePage() {
  ...
  const cardVariants = {
    ...
    visible: { opacity: 1, y: 0, transition: { duration: 0.7, type: 'spring' as AnimationGeneratorType } },
  };
  const stagger = {
    visible: {
      transition: {
  ...
  const QUESTION_TYPES = [
    { id: 'mcq-single', name: 'MCQ (Single)', icon: '🔘', guide: 'Multiple choice, one correct answer.' },
    { id: 'mcq-multiple', name: 'MCQ (Multiple)', icon: '☑️', guide: 'Multiple choice, multiple correct answers.' },
    { id: 'true-false', name: 'True/False', icon: '✅', guide: 'Simple true or false question.' },
    { id: 'match', name: 'Match Following', icon: '🔗', guide: 'Match items from two columns.' },
    { id: 'matrix', name: 'Matrix', icon: '📊', guide: 'Grid-based, select answers in a matrix.' },
    { id: 'poll', name: 'Poll', icon: '📈', guide: 'Survey-style, no correct answer.' },
    { id: 'paragraph', name: 'Paragraph', icon: '📝', guide: 'Short answer, manual grading.' },
    { id: 'fill-blanks', name: 'Fill Blanks', icon: '⬜', guide: 'Fill in the blanks in text.' },
    { id: 'code-output', name: 'Code Output', icon: '💻', guide: 'Predict output of code.' },
    { id: 'drag-drop', name: 'Drag & Drop', icon: '🖱️', guide: 'Arrange items by dragging.' },
    { id: 'image-based', name: 'Image Based', icon: '🖼️', guide: 'Answer based on an image.' },
    { id: 'audio', name: 'Audio', icon: '🎵', guide: 'Answer based on audio.' },
    { id: 'video', name: 'Video', icon: '🎬', guide: 'Answer based on video.' },
    { id: 'essay', name: 'Essay', icon: '✍️', guide: 'Long-form, manual grading.' },
    { id: 'ordering', name: 'Ordering', icon: '📋', guide: 'Arrange items in order.' },
  ];

  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-background to-white dark:from-[#0f1021] dark:via-[#23234d] dark:to-[#1a1a2e] py-12 px-4">
      {/* Animated Orbs */}
      <motion.div
      ...
      {/* Main Card */}
      <motion.div
        className="relative max-w-2xl w-full bg-white/90 text-blue-900 shadow-[0_0_24px_4px_rgba(59,130,246,0.15)] dark:bg-gray-900/80 dark:text-cyan-100 rounded-2xl shadow-2xl p-8 backdrop-blur-lg border border-cyan-400/30 ring-2 ring-cyan-400/20 ring-offset-2 ring-offset-blue-900/30 z-10"
      ...
      >
        ...
        >
          Create interactive quizzes with a variety of question types.<br />
          Save drafts, use templates, and publish your quizzes for others to explore!
        </motion.p>
        {/* Animated Info Box */}
        <motion.div
        ...
        >
          ...
          <span className="text-cyan-100 font-medium">
            Tip: You can import questions from templates or use AI to generate questions instantly!
          </span>
        </motion.div>
        <ol className="list-decimal list-inside text-blue-800 dark:text-cyan-200 mb-6 space-y-2 text-base">
          <li><b>Set Quiz Details:</b> Add a title, tags, and (optionally) a cover image.</li>
          <li><b>Add Questions:</b> Choose from many question types. Click "Add Question" and select a type to begin.</li>
          <li><b>Edit & Validate:</b> Fill in question details. The builder will guide you if anything is missing or invalid.</li>
          <li><b>Use Templates:</b> Import questions from your own or public templates to save time.</li>
          <li><b>Finalize & Publish:</b> When ready, finalize your quiz and publish it to make it available to others.</li>
        </ol>
        <div className="mb-8">
          <h2 className="text-2xl font-semibold text-blue-800 dark:text-cyan-200 mb-2">Supported Question Types</h2>
          <motion.div
          ...
          >
            {QUESTION_TYPES.map((type, idx) => (
              ...
              >
                <span className="text-3xl drop-shadow-glow group-hover:animate-bounce" aria-label={`${type.name} icon`}>{type.icon}</span>
                <span className="text-blue-900 dark:text-cyan-100 font-semibold text-lg">{type.name}</span>
                <span className="text-blue-800 dark:text-cyan-200 text-sm">{type.guide}</span>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      <style jsx global>{`
        ...
        @keyframes pulse-glow {
          0% { box-shadow: 0 0 16px #22d3ee44, 0 0 0 #fff0; }
          100% { box-shadow: 0 0 32px #22d3ee99, 0 0 8px #fff2; }
        }
        .animate-float {
