...
const FactoryQuizCard = ({ quiz, onSelect, hideImage = false }: { quiz: Quiz; onSelect: (quiz: Quiz) => void; hideImage?: boolean }) => (
  <motion.div
    initial={{ opacity: 0, y: 20 }}
    animate={{ opacity: 1, y: 0 }}
    exit={{ opacity: 0, y: -20 }}
    className={`rounded-2xl bg-gradient-to-br from-gray-100/40 dark:from-purple-900/40 to-gray-200/30 dark:to-blue-900/30 border border-gray-300/50 dark:border-white/10 shadow-xl p-5 ${hideImage ? 'flex-row items-center gap-6' : 'flex-col gap-2'} cursor-pointer hover:scale-[1.02] transition-all duration-300`}
    onClick={() => onSelect(quiz)}
  >
    {/* Image/Cover (hide in list view) */}
    {!hideImage && (
      quiz.imageUrl ? (
        <img src={quiz.imageUrl} alt={quiz.title} className="aspect-video w-full object-cover rounded-xl mb-2" />
      ) : (
    ...
    <div className="flex-1 min-w-0">
      <h3 className={`text-lg font-bold text-gray-900 dark:text-white/90 line-clamp-1 ${hideImage ? 'truncate max-w-[220px]' : ''}`}>{quiz.title}</h3>
      <p className="text-sm text-gray-600 dark:text-white/60 line-clamp-2">{quiz.description || "No description provided"}</p>
      <div className={`flex flex-wrap gap-2 mt-2 ${hideImage ? 'min-w-[120px]' : ''}`}>
      ...
      <div className={`${hideImage ? 'border-0 pt-0 mt-0 flex-col items-start gap-1' : 'mt-auto pt-3 border-t'} border-gray-300/50 dark:border-white/10 flex justify-between items-center text-sm text-gray-600 dark:text-white/60`}>
...
const CommentsDialog = ({ quiz, onClose }: { quiz: Quiz; onClose: () => void }) => {
    ...
    <Modal open={true} onClose={onClose} transparent>
      ...
      >
        <div className="flex items-center justify-between mb-6 flex-shrink-0">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white">Comments for "{quiz.title}"</h3>
          <button onClick={onClose} className="p-2 hover:bg-gray-100 dark:hover:bg-white/10 rounded-full transition-colors text-gray-600 dark:text-white">
            <FiX />
          </button>
        </div>
        <div className="space-y-4 overflow-y-auto pr-2 flex-1 max-h-[90vh]">
          {isLoading && !data && <p className="text-gray-700 dark:text-white">Loading comments...</p>}
          {error && <p className="text-red-500 dark:text-red-400">Failed to load comments.</p>}
          {isEmpty && <p className="text-gray-600 dark:text-white/60">No comments yet.</p>}
          {comments.map((comment: any) => (
            <div key={comment.id} className="bg-gray-50 dark:bg-white/5 p-4 rounded-lg flex gap-4">
              <img src={comment.user.avatarUrl || '/default_avatar.png'} alt={comment.user.name} className="w-10 h-10 rounded-full" />
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-gray-900 dark:text-white">{comment.user.name}</span>
                  <span className="text-xs text-gray-500 dark:text-white/50">{new Date(comment.createdAt).toLocaleDateString()}</span>
                </div>
                <p className="text-sm text-gray-700 dark:text-white/80 mt-1">{comment.text}</p>
              </div>
            </div>
          ))}
        </div>
...
const CreatorDashboard = ({ quiz, onClose, mutate }: { quiz: Quiz | null; onClose: () => void; mutate: () => void }) => {
  ...
  if (!quiz) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-gray-600 dark:text-white/60 p-2 sm:p-6">
        ...
        <p className="text-sm text-center">View statistics, manage comments, and control your published quizzes</p>
      </div>
    );
  }
  ...
  return (
    <div className="h-full flex flex-col p-2 sm:p-6">
      <div className="flex justify-between items-start mb-6">
        <h2 className="text-xl font-bold text-gray-900 dark:text-white/90">{quiz.title}</h2>
        <button
          onClick={onClose}
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-gray-50 dark:bg-white/5 rounded-xl p-4">
          <p className="text-sm text-gray-600 dark:text-white/60">Total Attempts</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-white/90">{quiz.usersTaken}</p>
        </div>
        <div className="bg-gray-50 dark:bg-white/5 rounded-xl p-4">
          <p className="text-sm text-gray-600 dark:text-white/60">Likes</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-white/90">{quiz.likeCount}</p>
        </div>
        <div className="bg-gray-50 dark:bg-white/5 rounded-xl p-4">
          <p className="text-sm text-gray-600 dark:text-white/60">Average Score</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-white/90">{quiz.averageScore.toFixed(1)}%</p>
        </div>
        <div className="bg-gray-50 dark:bg-white/5 rounded-xl p-4">
          <p className="text-sm text-gray-600 dark:text-white/60">Rating</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-white/90">⭐ {quiz.rating.toFixed(1)}</p>
        </div>
      </div>
      ...
      <AlertDialog open={isUnpublishAlertOpen} onOpenChange={setIsUnpublishAlertOpen}>
        <AlertDialogContent className="bg-white/90 dark:bg-[#1a1a2e]/90 backdrop-blur-lg border-gray-200 dark:border-white/10 text-gray-900 dark:text-white">
          <AlertDialogHeader>
            <AlertDialogTitle>Are you sure you want to unpublish this quiz?</AlertDialogTitle>
            <AlertDialogDescription>
              This will remove the quiz from the public explore page and move it back to your private templates. You can republish it at any time.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      {isCommentsDialogOpen && quiz && (
        <CommentsDialog quiz={quiz} onClose={() => setIsCommentsDialogOpen(false)} />
      )}
    </div>
  );
};
...
export default function FactoryDialog({ onClose }: { onClose: () => void }) {
  ...
  const { data: packages, mutate: mutatePackages } = useSWR(viewType === 'package' ? '/api/packages' : null, fetcher);
  ...
  const { data: selectedPackageStats, error: selectedPackageStatsError } = useSWR(
    selectedPackage ? `/api/packages/stats?id=${selectedPackage.id}` : null,
    fetcher
  );
  const [showCreateDialog, setShowCreateDialog] = useState(false);
  const [newPackage, setNewPackage] = useState({ title: '', description: '', imageUrl: '', quizIds: [], price: 0 });
  const [createLoading, setCreateLoading] = useState(false);
  const [canCreate, setCanCreate] = useState(true);
  const [pricingMode, setPricingMode] = useState<'free' | 'paid'>('free');
  const { data: allPublishedQuizzes, isLoading: loadingPublishedQuizzes } = useSWR('/api/quizzes/published', fetcher);
  ...
  const { data: quizzes, error, isLoading, mutate } = useSWR<Quiz[]>(
    `/api/quizzes/published?${params.toString()}`,
    fetcher
  );
  ...
  const handlePublishPackage = async (pkg, publish) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 404) {
          errorMsg = 'This package no longer exists or you do not have permission.';
        } else if (res.status === 400) {
          try {
            const data = await res.json();
            if (data?.error?.includes('Missing required fields')) {
              errorMsg = 'Please fill all required fields and add at least one quiz to your package.';
            } else if (data?.error) {
              errorMsg = data.error;
            }
          } catch (err) {
      ...
      if (isMounted.current) {
        mutatePackages();
        setSelectedPackage(prev => prev ? { ...prev, isPublished: publish } : null);
        toastId = toast.success(publish ? 'Package published!' : 'Package unpublished!');
      }
    } catch (err) {
      console.error('Publish error (network or unexpected):', err);
      if (isMounted.current) toastId = toast.error('Network error: Could not reach the server.');
    } finally {
  ...
  return (
    <Modal open={true} onClose={onClose} fullScreen>
          ...
          <div className="flex-1 flex flex-col p-2 sm:p-6 overflow-y-auto bg-gray-50/50 dark:bg-black/20">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="inline-flex rounded-full bg-gray-100 dark:bg-white/10 border border-gray-300 dark:border-white/20 shadow-sm overflow-hidden mr-4">
                <button
                  className={`px-4 py-2 flex items-center gap-2 transition-all ${viewType === 'quiz' ? 'bg-gradient-to-r from-purple-500 to-blue-500 text-white' : 'text-gray-600 dark:text-white/60 hover:bg-gray-200 dark:hover:bg-white/10'}`}
                ...
                <button
                  className={`px-4 py-2 flex items-center gap-2 transition-all ${viewType === 'package' ? 'bg-gradient-to-r from-purple-500 to-blue-500 text-white' : 'text-gray-600 dark:text-white/60 hover:bg-gray-200 dark:hover:bg-white/10'}`}
              ...
              <div className="inline-flex rounded-full bg-gray-100 dark:bg-white/10 border border-gray-300 dark:border-white/20 shadow-sm overflow-hidden">
                <button
                  className={`px-4 py-2 flex items-center gap-2 transition-all ${viewMode === 'grid' ? 'bg-gradient-to-r from-purple-500 to-blue-500 text-white' : 'text-gray-600 dark:text-white/60 hover:bg-gray-200 dark:hover:bg-white/10'}`}
                ...
                <button
                  className={`px-4 py-2 flex items-center gap-2 transition-all ${viewMode === 'list' ? 'bg-gradient-to-r from-purple-500 to-blue-500 text-white' : 'text-gray-600 dark:text-white/60 hover:bg-gray-200 dark:hover:bg-white/10'}`}
              ...
              {viewType === 'package' && (
                <Button
                  ...
                  style={{ boxShadow: '0 4px 24px 0 rgba(124,58,237,0.25), 0 1.5px 8px 0 rgba(236,72,153,0.15)' }}
                >
            ...
            {viewType === 'quiz' ? (
              <div className={
                ...
                {quizzes?.map(quiz => (
                  <div key={quiz.id} className={viewMode === 'list' ? 'w-full' : ''}>
                    <FactoryQuizCard quiz={quiz} onSelect={(q) => setSelectedQuiz(q)} hideImage={viewMode === 'list'} />
                  </div>
                ))}
              </div>
            ) : (
              <div className={
                ...
                {allPackages.map(pkg => {
                  const quizThumbs = (pkg.quizIds || []).slice(0, 3).map(qid => {
                    const quiz = (selectedPackageQuizzes || []).find(q => q.id === qid);
                    return quiz ? { imageUrl: quiz.imageUrl, title: quiz.title } : { title: "Quiz" };
                  });
                  return (
                    <div key={pkg.id} className={viewMode === 'list' ? 'w-full' : ''}>
                      <PackageCard pkg={{ ...pkg, quizThumbnails: quizThumbs }} onSelect={setSelectedPackage} hideImage={viewMode === 'list'} />
                    </div>
                  );
                })}
              </div>
            ...
            {viewType === 'quiz' ? (
              ...
              selectedPackage ? (
                <PackageDetailsPanel
                  ...
                  onPublish={async () => {
                    await handlePublishPackage(selectedPackage, !selectedPackage.isPublished);
                  }}
                ...
                <div className="flex items-center justify-center h-full text-gray-500 dark:text-white/40">Select a package to view details</div>
              )
            )}
          </div>
        </div>
      ...
      {showCreateDialog && (
        <Modal open={showCreateDialog} onClose={() => setShowCreateDialog(false)} wide={true}>
          <div className="bg-white/90 dark:bg-gradient-to-br from-[#181a20]/90 to-[#23243a]/90 border border-gray-200/50 dark:border-white/10 rounded-2xl shadow-2xl w-full max-w-4xl p-8 relative">
            <h2 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white text-center">Create a Quiz Package</h2>
            <button onClick={() => setShowCreateDialog(false)} className="absolute top-4 right-4 text-gray-500 dark:text-white/60 hover:text-gray-700 dark:hover:text-white/90 text-2xl">&times;</button>
            <div className="flex flex-col md:flex-row gap-8 w-full min-w-0">
              ...
              <div className="flex-1 min-w-0 bg-gray-50 dark:bg-black/10 rounded-xl p-4 border border-gray-200 dark:border-white/10">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Select Published Quizzes</h3>
                {loadingPublishedQuizzes ? (
                  <div className="text-gray-500 dark:text-white/50">Loading quizzes...</div>
                    ...
                    {allPublishedQuizzes.map(q => (
                      <li key={q.id} className="flex items-center gap-3">
                        <input
                          ...
                          id={`quiz-${q.id}`}
                        />
                        <div className="w-10 h-8 rounded-md overflow-hidden bg-gray-200 dark:bg-white/10 flex items-center justify-center">
                          {q.imageUrl ? (
                            <img src={q.imageUrl} alt={q.title} className="object-cover w-full h-full" />
                          ) : (
                            <span className="text-gray-400 dark:text-white/30">📄</span>
                          )}
                        </div>
                        <label htmlFor={`quiz-${q.id}`} className="text-gray-700 dark:text-white/80 cursor-pointer truncate max-w-[160px]">{q.title}</label>
                      </li>
                    ))}
                  </ul>
                ) : (
                  <div className="text-gray-500 dark:text-white/50">No published quizzes available.</div>
                )}
                <div className="mt-4 text-sm text-gray-600 dark:text-white/60">{newPackage.quizIds.length} quizzes selected</div>
              </div>
              {/* Right: Package details */}
              <div className="flex-1 min-w-0 flex flex-col gap-4">
                <label className="block text-gray-700 dark:text-white/80 font-semibold">Package Name</label>
                <input
                  type="text"
                  value={newPackage.title}
                  onChange={e => setNewPackage(np => ({ ...np, title: e.target.value }))}
                ...
                />
                <label className="block text-gray-700 dark:text-white/80 font-semibold">Description</label>
                <textarea
                  value={newPackage.description}
                  onChange={e => setNewPackage(np => ({ ...np, description: e.target.value }))}
                ...
                />
                <label className="block text-gray-700 dark:text-white/80 font-semibold">Image URL</label>
                <input
                  type="text"
                  value={newPackage.imageUrl}
                  onChange={e => setNewPackage(np => ({ ...np, imageUrl: e.target.value }))}
                  className="px-4 py-2 rounded-xl bg-white dark:bg-white/10 border border-gray-300 dark:border-white/20 text-gray-900 dark:text-white placeholder:text-gray-500 dark:placeholder:text-white/40 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm backdrop-blur-md w-full"
                  placeholder="Image URL (optional)"
                />
                {/* Package Pricing */}
                <div className="space-y-3">
                  <label className="block text-gray-700 dark:text-white/80 font-semibold">Package Pricing</label>
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      onClick={() => {
                        setPricingMode('paid');
                        setNewPackage(np => ({ ...np, price: np.price === 0 ? 100 : np.price }));
                      }}
                      className={`px-4 py-2 rounded-lg font-semibold transition-all ${
                        pricingMode === 'paid'
                    ...
                    <div className="relative">
                      <input
                        ...
                        onChange={e => {
                          ...
                          setNewPackage(np => ({ ...np, price: value * 100 }));
                        ...
                        step="1"
                      />
                      <span className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500 dark:text-white/60">₹</span>
                    </div>
                  )}
                </div>
                <div className="flex gap-4 mt-6 justify-end">
                  ...
                  <Button
                    ...
                    disabled={!newPackage.title || newPackage.quizIds.length === 0 || createLoading}
                    onClick={async () => {
                      ...
                      mutatePackages();
                      setNewPackage({ title: '', description: '', imageUrl: '', quizIds: [], price: 0 });
                      setPricingMode('free');
