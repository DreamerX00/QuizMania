...
export default function TemplateActionPanel({
  ...
  if (!quiz) {
    return (
      <div className="w-full h-full p-8 flex flex-col items-center justify-center text-center text-white/60">
        ...
        <p className="text-sm">
          Actions and a preview of your quiz will appear here.
        </p>
      </div>
    );
  }
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      await mutate(
        (currentQuizzes) => currentQuizzes?.filter((q) => q.id !== quiz.id),
        false
      );
      onClose();
    } catch (error) {
  ...
  const handlePin = async () => {
    setIsPinning(true);
    try {
      const { data: updatedQuiz } = await axios.patch(
        `/api/quizzes/templates/${quiz.id}/pin`
      );
      await mutate(
        (currentQuizzes) =>
    ...
    <>
      ...
      >
        ...
        <div className="space-y-4 text-sm">
          <div className="relative aspect-video bg-white/5 rounded-lg flex items-center justify-center overflow-hidden">
            {quiz.imageUrl ? (
              <Image
                src={quiz.imageUrl}
                alt={quiz.title}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <span className="font-semibold text-white/50 block">Price</span> ₹
              {quiz.price === 0 ? "Free" : quiz.price}
            </div>
            <div>
              <span className="font-semibold text-white/50 block">
                Questions
              </span>{" "}
              {getQuestionCount()}
            </div>
            <div>
              <span className="font-semibold text-white/50 block">Created</span>{" "}
              {format(new Date(quiz.createdAt), "dd MMM yyyy")}
            </div>
            <div>
              <span className="font-semibold text-white/50 block">Updated</span>{" "}
              {format(new Date(quiz.updatedAt), "dd MMM yyyy")}
            </div>
          </div>
        </div>
        ...
        <div className="mt-auto pt-6 flex flex-col gap-3">
          ...
          <div className="flex gap-3">
            <button
              onClick={handlePin}
              disabled={isPinning}
              className={`futuristic-button flex-1 flex items-center justify-center gap-2 disabled:opacity-50 ${
                quiz.isPinned ? "bg-yellow-500/80 text-white" : "bg-white/10"
              }`}
            >
              {isPinning ? (
                <Loader2 className="animate-spin" size={16} />
              ...
              {quiz.isPinned ? "Pinned" : "Pin"}
            </button>
            <button
              onClick={() => setShowDeleteConfirm(true)}
              disabled={isDeleting}
              className="futuristic-button flex-1 flex items-center justify-center gap-2 bg-red-800/50 text-red-300 hover:bg-red-800/80 disabled:opacity-50"
            >
              {isDeleting ? (
                <Loader2 className="animate-spin" size={16} />
              ) : (
      ...
      >
        <div className="text-center">
          ...
          <p className="text-white/60 mb-6">
            This will permanently delete the quiz template. This action cannot
            be undone.
          </p>
          <div className="flex justify-center gap-4">
            <button
              onClick={() => setShowDeleteConfirm(false)}
              className="px-6 py-2 rounded-lg bg-white/20 hover:bg-white/30"
            >
              Cancel
            </button>
            <button
              ...
              disabled={isDeleting}
              className="px-6 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white disabled:opacity-50"
            >
