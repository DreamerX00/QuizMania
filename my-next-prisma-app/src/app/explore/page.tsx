...
import FilterModalContent, {
  type FilterState,
} from "./components/FilterModalContent";
...
export default function ExplorePage() {
  ...
  const queryString = useMemo(() => {
    ...
    if (filters.quizType.length > 0)
      params.append("quizType", filters.quizType.join(","));
    if (filters.isFree !== null)
      params.append("isFree", String(filters.isFree));
  ...
  const filteredQuizzes = quizzes.filter((q: Quiz) => {
    ...
    if (
      filters.quizType.length > 0 &&
      (!q.tags ||
        !filters.quizType.some((type) =>
          q.tags
    ...
    if (
      ...
      new Date(q.createdAt) < new Date(filters.dateRange.from)
    )
      return false;
    if (
      filters.dateRange?.to &&
      new Date(q.createdAt) > new Date(filters.dateRange.to)
    )
      return false;
    return true;
  });
    ...
    <main className="min-h-screen bg-linear-to-br from-background to-white dark:from-[#0a0a0f] dark:via-[#1a1a2e] dark:to-[#16213e] text-foreground relative overflow-x-hidden">
      ...
      <section className="relative z-20 max-w-7xl mx-auto pt-16 pb-6 px-2 md:px-8 text-center flex flex-col items-center">
        ...
        <p className="text-base sm:text-xl md:text-2xl text-white/80 max-w-2xl mx-auto mb-8 font-medium futuristic-subtitle animate-fade-in-up">
          Discover, filter, and attempt the best quizzes from every field. Use
          tags and filters to find your next challenge!
        </p>
      </section>
      {/* Top Tags Bar */}
