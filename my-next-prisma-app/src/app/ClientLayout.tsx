...
const dockItems = [
  ...
  { title: 'Explore', icon: <Zap className="w-6 h-6" />, href: '/explore', restricted: true },
  { title: 'Create', icon: <Brain className="w-6 h-6" />, href: '/create-quiz/guide', restricted: true },
  { title: 'Leaderboard', icon: <Trophy className="w-6 h-6" />, href: '/leaderboard', restricted: true },
  { title: 'Profile', icon: <Users className="w-6 h-6" />, href: '/profile', restricted: true },
];

export default function ClientLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const isArenaPage = (pathname?.startsWith('/quiz/') && pathname?.includes('/take')) || pathname === '/multiplayer-arena';
