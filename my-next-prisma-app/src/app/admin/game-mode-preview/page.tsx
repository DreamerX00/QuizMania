...
export default function GameModePreviewPage() {
  const [selected, setSelected] = useState(schemaList[0].file);
    ...
    <div className="p-8 max-w-2xl mx-auto">
      ...
      <div className="mb-4">
        ...
        <select value={selected} onChange={e => setSelected(e.target.value)} className="border p-2 rounded">
          {schemaList.map(s => <option key={s.file} value={s.file}>{s.name}</option>)}
        </select>
      </div>
      {error && <div className="text-red-500">{error}</div>}
      {schema && <React.Suspense fallback={<div>Loading form...</div>}>
      ...
      <div className="mt-8 p-4 bg-gray-100 rounded">
        ...
        {rollbackResult && (
          <div className="mt-2">
            {rollbackResult.error && <div className="text-red-500">{rollbackResult.error}</div>}
            {rollbackResult.success && <div className="text-green-600">{rollbackResult.message}</div>}
          </div>
        )}
      </div>
    </div>
