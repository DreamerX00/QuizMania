...
export default function SchemaForm({ schema }: { schema: Schema }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const key in schema) {
      if (!schema[key].optional && (form[key] === undefined || form[key] === '')) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 bg-gray-50 p-4 rounded">
      ...
      <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded" disabled={loading}>{loading ? 'Validating...' : 'Validate (API)'}</button>
      {result && (
      ...
      {apiResult && (
        <div className="mt-4 bg-gray-100 p-2 rounded">
          ...
          {apiResult.error && <div className="text-red-500">{apiResult.error}</div>}
          {apiResult.valid && <div className="text-green-600">Valid!</div>}
          {apiResult.valid === false && (
            <div className="text-red-600">
              Invalid. Errors:
              <pre className="text-xs whitespace-pre-wrap">{JSON.stringify(apiResult.errors, null, 2)}</pre>
            </div>
          )}
        </div>
      )}
