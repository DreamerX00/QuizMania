...
import { Mic, Plus, Crown, Sparkles, Users, Sword, Target, Flame, Zap } from 'lucide-react';
      ...
      <div className="relative z-10 flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-white/60 dark:bg-slate-800/60 px-3 py-2 rounded-full backdrop-blur-sm border border-slate-200/50 dark:border-slate-700/50">
            ...
            <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">{filledSlots}/5</span>
          </div>
          <div className="flex items-center gap-2 bg-white/60 dark:bg-slate-800/60 px-3 py-2 rounded-full backdrop-blur-sm border border-slate-200/50 dark:border-slate-700/50">
            <Target className="w-4 h-4 text-green-600 dark:text-green-400" />
            <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">Ready</span>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-5 gap-4 w-full max-w-2xl">
          {participantsArray.length === 0 ? (
            <div className="col-span-5 flex flex-col items-center justify-center py-12">
              ...
              <p className="text-slate-500 dark:text-slate-400 font-medium">No warriors in arena</p>
              <p className="text-slate-400 dark:text-slate-500 text-sm">Invite players to begin the battle</p>
            </div>
          ) : (
            participantsArray.map((participant: any, index: number) => (
              ...
              >
                <div className="relative group">
                  <div className={`w-20 h-20 rounded-2xl flex items-center justify-center relative bg-gradient-to-br from-white to-slate-100 dark:from-slate-700 dark:to-slate-800 border-2 shadow-lg transition-all duration-300 group-hover:scale-105 ${
          ...
          {Array.from({ length: Math.max(0, 5 - participantsArray.length) }, (_, index) => (
            ...
            >
              ...
              <div className="text-center">
                <p className="text-sm font-medium text-slate-400 dark:text-slate-500">Empty</p>
                <p className="text-xs text-slate-400 dark:text-slate-500">Click to invite</p>
              </div>
            </motion.div>
          ))}
        </div>
