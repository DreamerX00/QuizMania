...
const ClanHubOverlay = ({ open, onClose }: ClanHubOverlayProps) => {
  // Assume your role is 'Leader' for now (can be dynamic later)
  const yourRole = 'Leader';
  return (
    <AnimatePresence>
      {open && (
        <motion.div
        ...
        >
          <div className="relative w-full max-w-[90vw] sm:max-w-[70vw] h-[80vh] bg-white dark:bg-[#16192a] rounded-2xl shadow-2xl flex flex-col p-0">
            <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex flex-col md:flex-row md:items-center md:justify-between gap-4 space-y-0">
              {/* Summary Card */}
              <div className="flex items-center gap-4">
                <img src={clanData.emblem} alt="emblem" className="w-16 h-16 rounded-lg border-2 border-slate-300 dark:border-slate-600" />
                <div>
                  <div className="text-2xl font-bold text-gray-900 dark:text-white">{clanData.name}</div>
                  <div className="text-slate-500 dark:text-slate-400 italic">{clanData.motto}</div>
                  <div className="mt-2 text-purple-400 font-semibold bg-purple-500/10 border border-purple-500/30 rounded-full px-4 py-1 w-fit">
                    Your Role: {yourRole}
                  </div>
                </div>
              </div>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                  <TabsContent value="chat" className="h-full flex flex-col">
                    ...
                    <div className="mt-4 flex gap-2">
                      ...
                      <Button className="bg-purple-600 hover:bg-purple-700 text-white"><Send size={18}/></Button>
                    </div>
                  </TabsContent>
                        ...
                        {joinRequests.map(req => (
                          <div key={req.name} className="flex items-center justify-between p-3 bg-slate-100 dark:bg-slate-800/60 rounded-lg border border-slate-200 dark:border-slate-700">
                            ...
                            <div className="flex gap-2">
                              <Button size="sm" variant="outline" className="text-green-600 dark:text-green-400 border-green-400 hover:bg-green-100 dark:hover:bg-green-900 hover:text-black">Accept</Button>
                              <Button size="sm" variant="outline" className="text-red-600 dark:text-red-400 border-red-400 hover:bg-red-100 dark:hover:bg-red-900 hover:text-black">Decline</Button>
                            </div>
                          </div>
                        ))}
                      </div>
