...
import { Vote, Puzzle, FileText, Mic, CheckSquare, Target, Zap, Flame, Clock, Trophy } from 'lucide-react';
    ...
    >
      ...
      <div className="relative z-10 grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 flex-1">
        {voteTypes.map((option, index) => (
          ...
          >
            {/* Icon */}
            <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br ${option.bgColor} border ${option.borderColor} flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}>
              <div className={`text-white bg-gradient-to-br ${option.color} bg-clip-text text-transparent`}>
                {option.icon}
              </div>
            </div>
            ...
            <div className="w-full bg-slate-200/80 dark:bg-slate-700/80 rounded-full h-2 relative overflow-hidden">
              <motion.div
                ...
                initial={{ width: 0 }}
                animate={{ width: `${(getVoteCount(option.name) / Math.max(1, totalVotes)) * 100}%` }}
                transition={{ duration: 0.8, ease: "easeOut" }}
              />
            </div>
      ...
      {isLoading && (
        <div className="relative z-10 flex items-center justify-center mt-4 p-4 bg-white/60 dark:bg-slate-800/60 rounded-2xl backdrop-blur-sm border border-slate-200/50 dark:border-slate-600/50">
          <div className="text-center">
            ...
            <p className="text-slate-600 dark:text-slate-400 text-sm">Loading votes...</p>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="relative z-10 flex items-center justify-center mt-4 p-4 bg-red-500/10 border border-red-500/30 rounded-2xl">
          <div className="text-center">
            ...
            <p className="text-red-600 dark:text-red-400 text-sm">Failed to load votes.</p>
          </div>
        </div>
      )}
