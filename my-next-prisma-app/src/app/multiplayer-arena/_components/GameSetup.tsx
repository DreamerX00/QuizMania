...
import { Users, Lock, Globe, UsersRound, Droplets, BrainCircuit, Globe2, Settings, Zap, Target, Shield } from 'lucide-react';
...
const gameModes = [
  ...
  { name: 'Custom', icon: <UsersRound size={20} />, description: 'Custom Rules' },
];

const difficulties = [
  { value: 'Easy', label: 'Easy', color: 'text-green-600', bg: 'bg-green-100 dark:bg-green-900/30' },
  { value: 'Medium', label: 'Medium', color: 'text-yellow-600', bg: 'bg-yellow-100 dark:bg-yellow-900/30' },
  { value: 'Hard', label: 'Hard', color: 'text-orange-600', bg: 'bg-orange-100 dark:bg-orange-900/30' },
  { value: 'Expert', label: 'Expert', color: 'text-red-600', bg: 'bg-red-100 dark:bg-red-900/30' },
];
...
const GameSetup = () => {
  ...
  return (
    ...
    >
      ...
      {/* Game Modes */}
      <div className="relative z-10 space-y-3">
        <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Battle Formation</label>
        <div className="grid grid-cols-2 gap-3">
          {gameModes.map((mode) => (
            ...
            >
              <div className={`p-2 rounded-lg ${selectedGameMode === mode.name ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-700'}`}>
              {mode.icon}
              </div>
              <div className="text-center">
      ...
      {/* Room Type */}
      <div className="relative z-10 space-y-3">
        <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Battle Access</label>
      <Tabs defaultValue="public" className="w-full">
          <TabList className="grid w-full grid-cols-3 bg-white/80 dark:bg-slate-800/80 border border-slate-300/50 dark:border-slate-600/50 rounded-xl p-1 backdrop-blur-sm">
            <Tab 
              id="private" 
              className={({isSelected}) => `flex items-center justify-center gap-2 p-3 rounded-lg cursor-pointer transition-all duration-300 ${
                isSelected 
        ...
        <div className="bg-gradient-to-r from-white/80 to-slate-50/80 dark:from-slate-800/80 dark:to-slate-700/80 rounded-2xl p-4 border border-slate-200/50 dark:border-slate-600/50 backdrop-blur-sm shadow-lg">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">Battle Impact</span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-center justify-between">
              <span className="text-xs text-slate-600 dark:text-slate-400">Coins</span>
              <span className="text-sm font-bold text-yellow-600 dark:text-yellow-400">+50</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-xs text-slate-600 dark:text-slate-400">Gems</span>
