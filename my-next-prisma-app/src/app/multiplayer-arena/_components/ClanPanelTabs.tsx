...
const DiscoverClansTab = () => {
  ...
  const { data: myRequestsData } = useSWR(
  ...
  return (
    <div>
      {/* ...search/filter UI... */}
      {isLoading ? (
        <div className="text-center text-slate-400 py-12">Loading clans...</div>
      ) : error ? (
...
const CreateClanTab = ({ onCreated }: { onCreated: (clan: Clan) => void }) => {
  ...
  const [emblemUrl, setEmblemUrl] = useState(
    "https://api.dicebear.com/8.x/icons/svg?seed=clan"
  );
    ...
    <form onSubmit={handleCreate} className="max-w-md mx-auto space-y-6">
      ...
      <div>
        ...
        <select
          ...
          className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-2 py-2"
        >
          {regions.map((r) => (
      ...
      <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Input
            ...
            placeholder="Paste image URL or use DiceBear"
          />
          <Button
            type="button"
            size="icon"
            ...
            onClick={() =>
              setEmblemUrl(
                `https://api.dicebear.com/8.x/icons/svg?seed=${Math.random()
                  .toString(36)
                  .substring(2, 8)}`
              )
            }
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="my">
          {myClan ? (
            <ClanModal
              isOpen={true}
              onClose={() => setMyClan(null)}
              clan={myClan}
            />
          ) : (
            <div className="text-center text-slate-500 py-12">
              You are not in a clan. Join or create one!
            </div>
          )}
        </TabsContent>
