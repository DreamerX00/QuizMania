...
import { Send, Users, PartyPopper, MessageCircle, Sparkles } from 'lucide-react';
...
const SocialChat = ({ friendId, roomId }: { friendId?: string; roomId?: string }) => {
  ...
  return (
    ...
    >
      ...
      <Tabs defaultValue={tab} className="w-full flex flex-col h-full relative z-10" onSelectionChange={setTab}>
        <TabList className="flex items-center gap-1 bg-white/60 dark:bg-slate-800/60 border border-slate-200/50 dark:border-slate-600/50 rounded-xl p-1 backdrop-blur-sm mb-4">
          <Tab 
            id="friends" 
            className={({isSelected}) => `flex items-center gap-2 py-2 px-4 cursor-pointer transition-all duration-300 rounded-lg ${
              isSelected 
