        ...
        <div className="flex-1 overflow-hidden">
          <Tabs defaultValue="overview" className="h-full flex flex-col">
            <TabsList className="mx-6 mt-4 bg-transparent border-b border-slate-200 dark:border-slate-800 rounded-none p-0">
              <TabsTrigger
                value="overview"
                className="data-[state=active]:bg-slate-100 dark:data-[state=active]:bg-slate-800/50 data-[state=active]:text-gray-900 dark:data-[state=active]:text-white text-slate-500 dark:text-slate-400"
              >
                Overview
              </TabsTrigger>
              <TabsTrigger
              ...
              <TabsContent value="overview">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-1 space-y-4">
                    ...
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span
                        ...
                        style={{ color: rankInfo.current.colorScheme[0] }}
                      >
                        {rankInfo.current.name}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      {rankInfo.next ? (
                        <span className="flex items-center gap-1 font-semibold">
                          <span className="text-lg">{rankInfo.next.emoji}</span>{" "}
                          {rankInfo.next.name}
                        </span>
                      ) : (
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      {rankInfo.next ? (
                        <span className="font-bold text-green-600 dark:text-green-400">
                          {(rankInfo.next.xpMin - userXP).toLocaleString()} XP
                        </span>
                      ) : (
                    ...
                    <div className="mt-4">
                      <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400 mb-1">
                        <span>
                          {rankInfo.current.xpMin.toLocaleString()} XP
                        </span>
                        <span>
                  ...
                  <div className="md:col-span-2 lg:col-span-1 space-y-4 flex flex-col">
                    ...
                    <div className="bg-transparent dark:bg-slate-800/50 p-4 rounded-lg border border-transparent dark:border-slate-700 flex-1">
                      <IQScoreGraph currentIQ={128} height={180} />
                    </div>
                    <div className="bg-transparent dark:bg-slate-800/50 p-4 rounded-lg border border-transparent dark:border-slate-700 h-48 flex items-center justify-center text-slate-500 dark:text-slate-400 flex-1">
                      Login Heatmap (Coming Soon)
                    </div>
                  </div>
                </div>
              </TabsContent>
              <TabsContent value="history">
                ...
                <div className="space-y-3">
                  {matchHistory.map((match) => (
                    <div
                      key={match.id}
                      className="flex justify-between items-center p-3 bg-slate-100 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700"
                    >
                      <div>
