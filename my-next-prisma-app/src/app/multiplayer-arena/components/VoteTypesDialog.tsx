...
const VOTE_TYPES = [
  ...
  },
  {
    ...
    icon: <Image className="w-5 h-5" aria-label="Image Based Quiz" />,
...
const VoteTypesDialog: React.FC<VoteTypesDialogProps> = ({
  isOpen,
  onClose,
}) => {
        ...
        <div className="relative z-10 p-6 flex-1 overflow-y-auto">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6">
            {VOTE_TYPES.map((t, index) => (
              ...
              >
                <div
                  className={`rounded-2xl bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm border ${t.borderColor} p-4 sm:p-5 flex flex-col items-center justify-between shadow-lg hover:shadow-xl transition-all duration-300 h-32 sm:h-36`}
                >
                  {/* Icon */}
                  <div
                    className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-linear-to-br ${t.bgColor} border ${t.borderColor} flex items-center justify-center group-hover:scale-110 transition-transform duration-300 mb-3`}
                  >
                    <div
                      className={`text-white bg-linear-to-br ${t.color} bg-clip-text text-transparent`}
                    >
                      {t.icon}
                    </div>
                  </div>
