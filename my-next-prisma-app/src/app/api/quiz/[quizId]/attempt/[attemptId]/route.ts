...
export const GET = withValidation(paramsSchema, async (request: any, { params }: { params: { quizId: string, attemptId: string } }) => {
  ...
  const revisedMarks = manualReviews.reduce((sum, r) => sum + (r.marksAwarded ?? 0), 0);
  const allReviewed = manualReviews.length > 0 && manualReviews.every(r => r.reviewed);
