...
export async function POST(request: NextRequest) {
  ...
  const userId = currentUser?.id;
    ...
    if (action === 'send') {
      ...
      if (!clanId) return NextResponse.json({ error: 'Clan ID required' }, { status: 400 });
      // Check not already a member or requested
      const existing = await prisma.clanMembership.findFirst({ where: { clanId, userId } }) ||
        await prisma.clanJoinRequest.findFirst({ where: { clanId, userId, status: 'PENDING' } });
      if (existing) return NextResponse.json({ error: 'Already a member or requested' }, { status: 409 });
      const req = await prisma.clanJoinRequest.create({ data: { clanId, userId, status: 'PENDING' } });
      ...
      const joinReq = await prisma.clanJoinRequest.findUnique({ where: { id: requestId }, include: { clan: true } });
      if (!joinReq) return NextResponse.json({ error: 'Request not found' }, { status: 404 });
      const membership = await prisma.clanMembership.findFirst({ where: { clanId: joinReq.clanId, userId, role: { in: ['LEADER', 'ELDER'] } } });
      if (!membership) return NextResponse.json({ error: 'Not authorized' }, { status: 403 });
      // Accept: add to clan, update request
      if (action === 'accept') {
        await prisma.clanMembership.create({ data: { clanId: joinReq.clanId, userId: joinReq.userId, role: 'MEMBER' } });
      }
      const updated = await prisma.clanJoinRequest.update({ where: { id: requestId }, data: { status: action === 'accept' ? 'ACCEPTED' : 'DECLINED' } });
