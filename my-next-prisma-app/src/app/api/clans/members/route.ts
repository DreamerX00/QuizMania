...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to list clan members" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!clanId || !newUserId) {
      return NextResponse.json(
        { error: "Clan ID and user ID required" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { error: "Failed to add clan member" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await prisma.clanMembership.deleteMany({
      where: { clanId, userId: removeUserId },
    });
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("Error removing clan member:", error);
    return NextResponse.json(
      { error: "Failed to remove clan member" },
      { status: 500 }
    );
  }
}
