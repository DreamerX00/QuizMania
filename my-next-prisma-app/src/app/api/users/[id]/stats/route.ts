...
export const GET = withValidation(
  { params: statsParamsSchema, query: statsQuerySchema },
  async (request, _context) => {
    const { id: userId } = request.validatedParams!;
    const { page, limit: rawLimit } = request.validatedQuery!;
    ...
    try {
      ...
      const averageScore =
        allAttempts.length > 0
          ? Math.round(totalScore / allAttempts.length)
          : 0;
      const highestScore =
        allAttempts.length > 0
