...
import { z } from 'zod';
import { withValidation, sanitizeObject } from '@/utils/validation';
...
export const POST = async (request: NextRequest) => {
  try {
    ...
    if (!result.success) {
      return NextResponse.json({ error: 'Invalid webhook payload', details: result.error.errors }, { status: 400 });
    }
