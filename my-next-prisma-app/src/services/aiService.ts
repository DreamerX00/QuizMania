 ...
 */
export const generateQuizWithDeepSeek = async (
  ...
  try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a quiz generation expert who creates high-quality questions.",
            },
            {
              role: "user",
              content: prompt,
    ...
    quizData.questions.forEach((question, index) => {
      ...
      if (/[`'"()]/.test(question.explanation)) {
        throw new Error(
          `Explanation in question ${index + 1} contains invalid characters`
        );
      }
...
export const generateQuizWithFallback = async (
  ...
  const providers = [preferredProvider, "openai", "deepseek", "gemini"].filter(
    (v, i, a) => a.indexOf(v) === i
  );
