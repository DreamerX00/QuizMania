...
// Clan Chat
export const createClanChat = async (
  clanId: string,
  userId: string,
  message: string
) => {
  return prisma.clanChat.create({
...
// Friend Chat
export const createFriendChat = async (
  senderId: string,
  receiverId: string,
  message: string
) => {
...
export const deleteAllUserMessages = async (userId: string) => {
  await prisma.clanChat.deleteMany({ where: { senderId: userId } });
  await prisma.roomChat.deleteMany({ where: { senderId: userId } });
