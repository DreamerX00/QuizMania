...
export interface VoiceEvent {
  user: SocketUser;
  roomId: string;
  action: 'joined' | 'left' | 'muted' | 'unmuted' | 'speaking' | 'stopped-speaking';
...
class SocketService {
  ...
  private setupEventListeners() {
    ...
    this.socket.on('voice:livekit-join', (data: { token: string; roomId: string }) => {
  ...
  pushToTalk(roomId: string, speaking: boolean) {
    ...
    this.socket.emit('voice:push-to-talk', { roomId, speaking }, (response: any) => {
      if (response?.error) {
