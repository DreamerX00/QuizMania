...
export function useVoiceChat({
  ...
  useEffect(() => {
    ...
    // Voice events
    socket.on('voice:livekit-join', ({ token, roomId }) => {
      setState(prev => ({ 
        ...prev, 
        mode: 'livekit',
        roomId 
    ...
    socket.on('voice:user-joined', ({ user, mode }) => {
      setState(prev => ({
        ...prev,
    ...
    socket.on('voice:user-muted', ({ user, muted }) => {
      setState(prev => ({
        ...prev,
    ...
    socket.on('voice:user-speaking', ({ user, speaking }) => {
      setState(prev => ({
        ...prev,
        participants: prev.participants.map(p => 
    ...
    socket.on('voice:fallback-activated', ({ reason, mode }) => {
    ...
    socket.on('webrtc:peer-joined', ({ userId: peerUserId, socketId }) => {
      setState(prev => ({
        ...prev,
        participants: [...prev.participants, {
          userId: peerUserId,
    ...
    socket.on('webrtc:peer-left', ({ userId: peerUserId }) => {
      setState(prev => ({
        ...prev,
        participants: prev.participants.filter(p => p.userId !== peerUserId)
      }));
    });

    socket.on('webrtc:peer-muted', ({ userId: peerUserId, muted }) => {
      setState(prev => ({
        ...prev,
        participants: prev.participants.map(p => 
    ...
    socket.on('webrtc:peer-speaking', ({ userId: peerUserId, speaking }) => {
      setState(prev => ({
        ...prev,
        participants: prev.participants.map(p => 
