  ...
  const rendered = parts.map((part, i) => {
    if (part.match(/^___+$/)) {
      ...
      return (
        <input
          key={i}
          className="inline-block w-32 px-2 py-1 mx-1 rounded bg-white/10 border-b-2 border-(--primary-accent) text-white focus:outline-none"
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="font-heading text-2xl font-bold mb-2 pb-4 border-b border-muted-foreground/20">
        <span className="inline-block">{rendered}</span>
      </div>
