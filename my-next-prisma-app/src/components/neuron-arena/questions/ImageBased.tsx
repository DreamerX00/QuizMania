...
const ImageBased = ({ question }: { question: Question }) => {
  ...
  const [selected, setSelected] = useState<string | null>(
    question.options ? (typeof prev === "string" ? prev : null) : null
  );
  const [input, setInput] = useState(
    question.options ? "" : String(prev ?? "")
  );
  useEffect(() => {
    if (question.options) {
      ...
      if (!isEqual(input, prev)) setInput(String(prev ?? ""));
    }
        ...
        {question.options ? (
          (
            ...
            <button
              ...
              aria-pressed={selected === (opt.id || String(i))}
            >
