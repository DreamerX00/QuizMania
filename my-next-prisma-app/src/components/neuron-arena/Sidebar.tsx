...
function getQuestionState(i: number, responses: any[], quiz: any) {
  ...
  if (q.type === 'poll' || q.type === 'essay' || q.type === 'paragraph' || q.type === 'audio' || q.type === 'video') {
    return 'answered'; // manual review or poll, just mark as answered
  }
  if (q.correctAnswer !== undefined) {
    if (JSON.stringify(resp.response) === JSON.stringify(q.correctAnswer)) return 'correct';
    ...
    <aside className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-5 gap-2 mb-4">
        {quiz.questions.map((q, i) => {
          ...
          function isNonEmptyResponse(r) {
            if (!r) return false;
            const val = r.response;
            if (val === undefined || val === null) return false;
            if (typeof val === 'string') return val.trim().length > 0;
            if (Array.isArray(val)) return val.some(v => (typeof v === 'string' ? v.trim().length > 0 : v != null && v !== ''));
            if (typeof val === 'object') return Object.values(val).some(v => v != null && v !== '' && (typeof v !== 'string' || v.trim().length > 0));
            return true;
          }
          ...
          if (submitted && state === 'incorrect') tooltip += ' (Incorrect)';
          if (submitted && state === 'answered') tooltip += ' (Answered)';
          if (!submitted && !isMarked && !isCurrent && state === 'unanswered') tooltip += ' (Unanswered)';
          return (
            <motion.button
              key={q.id}
              ...
              animate={isCurrent ? { scale: 1.18, boxShadow: '0 0 0 4px rgba(127,90,240,0.25)' } : { scale: 1, boxShadow: 'none' }}
              transition={{ type: 'spring', stiffness: 400, damping: 20 }}
              layout
            >
              {i + 1}
              {isMarked && <span className="absolute -top-1 -right-1 w-3 h-3 bg-yellow-400 rounded-full border-2 border-background" />}
            </motion.button>
          );
        })}
      </div>
      ...
      <div className="flex items-center gap-3 text-sm flex-wrap">
        <span className="inline-block w-4 h-4 rounded-full border-2 border-primary mx-1"></span> Current
        <span className="inline-block w-4 h-4 rounded-full border-2 border-yellow-400 mx-1"></span> Marked
        <span className="inline-block w-4 h-4 rounded-full border-2 border-green-500 mx-1"></span> Answered
        {submitted && <><span className="inline-block w-4 h-4 rounded-full border-2 border-green-500 mr-1"></span> Correct
        <span className="inline-block w-4 h-4 rounded-full border-2 border-red-500 mx-1"></span> Incorrect
        <span className="inline-block w-4 h-4 rounded-full border-2 border-blue-400 mx-1"></span> Answered</>}
      </div>
    </aside>
  );
};
