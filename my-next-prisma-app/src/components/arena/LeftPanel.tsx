...
const LeftPanel: React.FC = () => {
  ...
  const [impact, setImpact] = useState({ xp: 120, rank: 2, coins: 50, gems: 1, gain: true });
  ...
  return (
    ...
    >
      <SignedIn>
        ...
        <div className="mb-6">
          <label htmlFor="difficulty-select" className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
            Difficulty Level
          </label>
          <select
          ...
          >
            {DIFFICULTY_LEVELS.map(level => (
              <option key={level.value} value={level.value} className="bg-white dark:bg-zinc-900 text-gray-900 dark:text-white">
                {level.label}
              </option>
            ))}
          </select>
            ...
            {GAME_MODES.map((mode) => (
              ...
              >
                ...
                <span className="flex gap-1">
                  {[...Array(mode.value === "CUSTOM" ? customSlots : mode.slots)].map((_, i) => (
                    <span key={i} className="w-2 h-2 rounded-full bg-gradient-to-br from-blue-400 to-cyan-300 dark:from-blue-500 dark:to-cyan-500 animate-pulse" />
                  ))}
                </span>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl font-bold text-xs border-2 transition-all
                  ${roomType === type.value
                    ? "bg-blue-600 dark:bg-blue-500 text-white border-blue-400 animate-pulse"
                    : "bg-white/60 dark:bg-zinc-800/60 text-gray-700 dark:text-gray-200 border-gray-200/50 dark:border-white/20 hover:bg-blue-100 dark:hover:bg-blue-700/40"}
                  ${type.premium ? "relative after:content-['PREMIUM'] after:absolute after:-top-2 after:right-2 after:bg-yellow-400 after:text-xs after:px-2 after:py-0.5 after:rounded after:shadow-md after:text-yellow-900" : ""}
                  ${styles["neon-glass"]}
                `}
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 text-xs font-mono text-green-500 animate-pulse">12ms</span>
          </div>
        </div>
        {/* Impact Indicator */}
        <div className="mb-6">
          ...
          <div className={`flex flex-col gap-2 p-4 rounded-2xl border-2 shadow-lg bg-white/80 dark:bg-zinc-900/80 ${styles["neon-glass"]}`}>
            <div className="flex items-center gap-3">
              <span className="text-green-500 text-lg font-bold animate-bounce">{impact.gain ? "+" : "-"}{impact.xp} XP</span>
              <span className="text-blue-500 text-lg font-bold animate-bounce">{impact.gain ? "+" : "-"}{impact.rank} Rank</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-yellow-500 text-lg font-bold animate-bounce">{impact.gain ? "+" : "-"}{impact.coins} Coins</span>
              <span className="text-pink-500 text-lg font-bold animate-bounce">{impact.gain ? "+" : "-"}{impact.gems} Gems</span>
            </div>
          </div>
        </div>
      </SignedIn>
      <SignedOut>
        <div className="flex flex-col items-center justify-center h-full">
          <p className="text-lg text-gray-700 dark:text-gray-300 mb-4">Sign in to access the Multiplayer Arena</p>
