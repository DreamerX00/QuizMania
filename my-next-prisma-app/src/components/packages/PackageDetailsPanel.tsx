...
import { BookOpen, Edit, Trash2, X, UploadCloud, Upload, Download } from "lucide-react";
...
const PackageDetailsPanel: React.FC<PackageDetailsPanelProps> = ({ pkg, quizzes, isEditing: initialEditing = false, onClose, onUpdate, onDelete, onPublish, stats, statsError, availableQuizzes = [], publishLoading = false }) => {
  ...
  const [pricingMode, setPricingMode] = useState<'free' | 'paid'>(pkg.price === 0 ? 'free' : 'paid');
  ...
  const addableQuizzes = availableQuizzes.filter(q => !form.quizIds.includes(q.id));
    ...
    <div className="w-full h-full flex flex-col bg-white/80 dark:bg-black/20 border border-white/10 rounded-2xl p-6 md:p-8 overflow-y-auto">
      <div className="flex justify-between items-center mb-4">
        {isEditing ? (
          ...
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white truncate max-w-[260px]">{pkg.title}</h2>
        )}
      ...
      {statsError && statsError.status === 404 ? (
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div>
            <div className="text-xs text-gray-500 dark:text-white/60 mb-1">Total Attempts</div>
            <div className="text-xl font-bold text-gray-900 dark:text-white">{stats.attempts}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500 dark:text-white/60 mb-1">Likes</div>
            <div className="text-xl font-bold text-gray-900 dark:text-white">{stats.likes}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500 dark:text-white/60 mb-1">Earnings</div>
            <div className="text-xl font-bold text-green-600 dark:text-green-400">₹{stats.earnings}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500 dark:text-white/60 mb-1">Price</div>
            <div className="text-xl font-bold text-blue-600 dark:text-blue-400">{pkg.price === 0 ? 'Free' : `₹${Math.floor(pkg.price / 100)}`}</div>
          </div>
          {stats.averageRating !== undefined && (
            <div>
              <div className="text-xs text-gray-500 dark:text-white/60 mb-1">Rating</div>
              <div className="text-xl font-bold text-yellow-500 dark:text-yellow-400">⭐ {stats.averageRating}</div>
            </div>
          )}
          {stats.averageScore !== undefined && (
            <div>
              <div className="text-xs text-gray-500 dark:text-white/60 mb-1">Avg Score</div>
              <div className="text-xl font-bold text-purple-500 dark:text-purple-400">{stats.averageScore}%</div>
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        <div className="bg-gray-100 dark:bg-white/10 text-xs px-2 py-1 rounded-full text-gray-800 dark:text-white">
          <span className="font-semibold">Status:</span> {pkg.isPublished ? 'Published' : 'Draft'}
        </div>
        <div className="bg-gray-100 dark:bg-white/10 text-xs px-2 py-1 rounded-full text-gray-800 dark:text-white">
          <span className="font-semibold">Type:</span> {pkg.price === 0 ? 'Free' : 'Paid'}
        </div>
      </div>
      <div className="space-y-4 text-sm">
        ...
        <div className="w-full aspect-[16/9] mb-4 rounded-2xl overflow-hidden border border-white/10 shadow-lg">
          {pkg.imageUrl ? (
            <img src={form.imageUrl || pkg.imageUrl} alt={pkg.title} className="object-cover w-full h-full" />
          ) : (
            <div className="w-full h-full flex items-center justify-center text-5xl font-bold text-gray-300 dark:text-white/40">IMG</div>
          )}
        </div>
        ...
        {/* Description */}
        <div>
          <label className="text-xs text-gray-600 dark:text-white/60">Description</label>
          {isEditing ? (
            <textarea
              name="description"
            ...
            <p className="text-gray-900 dark:text-white min-h-[40px]">{pkg.description || 'No description provided.'}</p>
          )}
        </div>
        {/* Package Pricing */}
        <div>
          <label className="text-xs text-gray-600 dark:text-white/60">Package Pricing</label>
          {isEditing ? (
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  onClick={() => {
                    setPricingMode('paid');
                    setForm(f => ({ ...f, price: f.price === 0 ? 100 : f.price }));
                  }}
                  className={`px-3 py-1 rounded text-sm font-semibold transition-all ${
                    pricingMode === 'paid'
              ...
              {pricingMode === 'paid' && (
                <div className="relative">
                  <input
                    ...
                    step="1"
                  />
                  <span className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-600 dark:text-white/60">₹</span>
                </div>
              )}
            </div>
          ) : (
            <p className="text-gray-900 dark:text-white">{pkg.price === 0 ? 'Free Package' : `₹${Math.floor(pkg.price / 100)}`}</p>
          )}
        </div>
        {/* Created Date */}
        <div className="text-xs text-gray-500 dark:text-white/60">Created: {new Date(pkg.createdAt).toLocaleDateString()}</div>
        {/* Quizzes List */}
        <div>
          <label className="text-xs text-gray-600 dark:text-white/60 mb-2 block">Quizzes in this package:</label>
          <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
            {quizzes.length === 0 && <div className="text-gray-400 dark:text-white/40">No quizzes in this package.</div>}
            {quizzes.map(q => (
              <div key={q.id} className="flex items-center gap-3 bg-white/5 rounded-lg px-2 py-1">
                ...
                <span className="flex-1 truncate text-gray-900 dark:text-white text-sm">{q.title}</span>
                {/* Remove button for draft packages */}
                {!pkg.isPublished && (
                  <button className="p-1 rounded hover:bg-red-500/20 transition-colors" title="Remove Quiz" onClick={() => handleRemoveQuiz(q.id)}>
                    <Trash2 size={16} />
                  </button>
                )}
              </div>
              ...
              {showAddModal && (
                <Modal open={showAddModal} onClose={() => setShowAddModal(false)}>
                  <div className="bg-[#181a20] rounded-2xl p-6 w-full max-w-md mx-auto">
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4">Add Quizzes to Package</h3>
                    <div className="max-h-64 overflow-y-auto space-y-2 mb-4">
                      {addableQuizzes.map(q => (
                        <label key={q.id} className="flex items-center gap-3 bg-white/5 rounded-lg px-2 py-2 cursor-pointer">
                          <input
                            type="checkbox"
                      ...
                      {addableQuizzes.length === 0 && <div className="text-gray-400 dark:text-white/40">No quizzes available to add.</div>}
                    </div>
        ...
        {isEditing ? (
          ...
          <>
            ...
            {pkg.isPublished ? (
              ...
              <>
                <button onClick={() => setIsEditing(true)} className="px-4 py-2 rounded-lg bg-white/10 text-gray-900 dark:text-white hover:bg-white/20 flex items-center gap-2"><Edit size={16}/> Edit</button>
